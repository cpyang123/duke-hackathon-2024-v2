...
# Enhanced search bar with popular searches
st.text_input("Search for research area...", placeholder="Cancer research", key="search_query")
if st.button("Start search"):
    search_query = st.session_state["search_query"]
    st.write(f"Searching for: {search_query}")

st.markdown("### Popular Searches")
popular_searches = ["Machine learning", "AI", "Robotics", "Climate science"]
...
with col1:
    if st.button("Home"):
        st.write("Home selected")

with col2:
    if st.button("Matches"):
        st.write("Matches selected")

with col3:
    if st.button("Profile"):
        st.write("Profile selected")
